...
const PokemonList = () => {
    ...
    const fetchPokemon = async (type = "") => {
      ...
      try {
        ...
        const responseType = await axios.get(
          `${process.env.REACT_APP_BACKEND_URL}/pokemon/type`,
        );
  ...
  useEffect(() => {
    ...
    const fetchPokeData = async () => {
      const paginatedListWithData = await Promise.all(
        paginatedList.map(async (pl) => {
          try {
            const res = await axios.get(
              `${process.env.REACT_APP_BACKEND_URL}/pokemon/${pl.name}`,
            );
            return { ...res.data };
          } catch (err) {
  ...
  const filterByType = async (type) => {
    ...
    try {
      const resp = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/pokemon?type=${type}`,
      );
  ...
  const isLikedPokemon = (pokemonId) => {
    const liked = userLikes.find(
      (l) => Number(l.pokemon_id) === Number(pokemonId),
    );
  ...
  const likePokemon = async (pokemonId, userId) => {
    try {
      ...
      const newUserLikes = [
        ...userLikes,
        { id: res.data[0].id, user_id: userId, pokemon_id: pokemonId },
      ];
      setUserLikes(newUserLikes);
    } catch (err) {
          ...
          {paginatedList.map((p) => (
            <li key={p.id} className="list-group-item p-3">
              <div className="container vstack border border-secondary rounded">
                <h3
                  className="text text-capitalize mt-3 mb-0"
